...
void main() {
  ...
  group('CustomerFleetIntent Domain Model', () {
    ...
    test('same model multiple intent identifies as sameVehicleMultiple (e.g. 7 x Innova)', () {
      ...
      expect(intent.bookingType, equals(FleetBookingType.sameVehicleMultiple));
    ...
    test('mixed fleet intent identifies as mixedFleet (4 Innova, 3 Camry, 1 BMW)', () {
  ...
  group('Fleet Availability Check (No Silent Substitution)', () {
    test('returns available when requested count is within mock inventory', () async {
      final intent = CustomerFleetIntent.sameModelMultiple(
        passengerCount: 18,
        model: 'Toyota Innova Crysta',
    ...
    test('reports shortfall and provides explicit alternatives when inventory is insufficient (7 x Innova vs 5 available)', () async {
      final intent = CustomerFleetIntent.sameModelMultiple(
        passengerCount: 40,
        model: 'Toyota Innova Crysta',
    ...
    test('mixed fleet detects partial shortfall when a model exceeds inventory', () async {
      final intent = CustomerFleetIntent.mixed(
        passengerCount: 50,
        units: {
  ...
  group('Parent Group Booking & Vehicle Assignments Persistence', () {
    test('submits same-model group booking (7 x Innova) with individual vehicle assignments', () async {
      final intent = CustomerFleetIntent.sameModelMultiple(
        passengerCount: 40,
        model: 'Toyota Innova Crysta',
        count: 7,
      ...
      // Verify retrieval
      final fetchResult = await repo.getGroupBooking(groupBooking.parentBookingId);
      expect(fetchResult.isSuccess, isTrue);
      expect(fetchResult.dataOrNull?.parentBookingId, equals(groupBooking.parentBookingId));
      expect(fetchResult.dataOrNull?.assignments.length, equals(7));
    });

    test('submits mixed fleet group booking (4 Innova, 3 Camry, 1 BMW) with correct model distribution', () async {
      final intent = CustomerFleetIntent.mixed(
        passengerCount: 40,
        units: {
          'Toyota Innova Crysta': 4,
    ...
    test('idempotency ensures duplicate submission returns cached parent group booking', () async {
      final intent = CustomerFleetIntent.single(
        passengerCount: 4,
        model: 'BMW 5 Series',
      );
      ...
      expect(first.dataOrNull!.parentBookingId, equals(second.dataOrNull!.parentBookingId));
      expect(first.dataOrNull!.bookingReference, equals(second.dataOrNull!.bookingReference));
    });
  });
}
